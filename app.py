...
'''
En este código, se está creando una instancia de la clase Flask y se está configurando para
permitir el acceso cruzado entre dominios utilizando el módulo CORS.
...
CORS(app): Se utiliza el módulo CORS para habilitar el acceso cruzado entre dominios en la
aplicación Flask. Esto significa que el backend permitirá solicitudes provenientes de dominios
diferentes al dominio en el que se encuentra alojado el backend. Esto es útil cuando se
desarrollan aplicaciones web con frontend y backend separados, ya que permite que el frontend
acceda a los recursos del backend sin restricciones de seguridad del navegador. Al pasar app
como argumento a CORS(), se configura CORS para aplicar las políticas de acceso cruzado a la
aplicación Flask representada por app.
...
'''
En este código, se están configurando la base de datos y se están creando objetos para
interactuar con ella utilizando SQLAlchemy y Marshmallow.

app.config["SQLALCHEMY_DATABASE_URI"]: Se configura la URI (Uniform Resource Identifier)
de la base de datos. En este caso, se está utilizando MySQL como el motor de base de datos,
el usuario y la contraseña son "root", y la base de datos se llama "proyecto". Esta
configuración permite establecer la conexión con la base de datos.

app.config["SQLALCHEMY_TRACK_MODIFICATIONS"]: Se configura el seguimiento de modificaciones
de SQLAlchemy. Al establecerlo en False, se desactiva el seguimiento automático de
modificaciones en los objetos SQLAlchemy, lo cual mejora el rendimiento.

db = SQLAlchemy(app): Se crea un objeto db de la clase SQLAlchemy, que se utilizará para
interactuar con la base de datos. Este objeto proporciona métodos y funcionalidades para
realizar consultas y operaciones en la base de datos.

ma = Marshmallow(app): Se crea un objeto ma de la clase Marshmallow, que se utilizará para
serializar y deserializar objetos Python a JSON y viceversa. Marshmallow proporciona una
forma sencilla de definir esquemas de datos y validar la entrada y salida de datos en la aplicación.
Este objeto se utilizará para definir los esquemas de los modelos de datos en la aplicación.
...
#app.config["SQLALCHEMY_DATABASE_URI"] = "mysql+pymysql://root:14cab056CAB411@localhost/proyecto"
#app.config["SQLALCHEMY_DATABASE_URI"] = "mysql+pymysql://root:14cab056CAB411@localhost/proyecto"

#CONEXION PYTHONANYWHERE
app.config["SQLALCHEMY_DATABASE_URI"] = "mysql+pymysql://JOSE14056411:claveproyecto@JOSE14056411.mysql.pythonanywhere-services.com/JOSE14056411$proyecto"

'''
CONEXION BASE DE DATOS

DB_URI = "mysql+pymysql://{username}:{password}@{hostname}/{databasename}".formaT(
    username="JOSE14056411"
    password="claveproyecto"
    hostname="JOSE14056411.mysql.pythonanywhere-services.com"
    databasename="JOSE14056411$proyecto"
)
...
#    id = db.Column(db.Integer, primary_key=True)
    ...
    def __init__(self, mail, nombre, password):
        """
...
'''
Este código define un endpoint que permite obtener todos los usuarios de la base de datos y los
devuelve como un JSON en respuesta a una solicitud GET a la ruta /usuarios.
@app.route("/usuarios", methods=["GET"]): Este decorador establece la ruta /usuarios para este
endpoint y especifica que solo acepta solicitudes GET.
def get_Usuarios(): Esta es la función asociada al endpoint. Se ejecuta cuando se realiza una
solicitud GET a la ruta /usuarios.
all_usuarios = Usuario.query.all(): Se obtienen todos los registros de la tabla de usuarios
mediante la consulta Usuario.query.all(). Esto se realiza utilizando el modelo Usuario que
...
result = usuarios_schema.dump(all_usuarios): Los registros obtenidos se serializan en
formato JSON utilizando el método dump() del objeto usuarios_schema. El método dump() heredado
de ma.Schema se utiliza para convertir los objetos Python en representaciones JSON.
return jsonify(result): El resultado serializado en formato JSON se devuelve como respuesta al
cliente utilizando la función jsonify() de Flask. Esta función envuelve el resultado en una
respuesta HTTP con el encabezado Content-Type establecido como application/json.
...
'''
El código que sigue a continuación termina de resolver la API de gestión de usuarios, a
continuación se destaca los principales detalles de cada endpoint, incluyendo su funcionalidad y
...
update_usuario(id):
    ...
    # Lee los datos proporcionados en formato JSON por el cliente y actualiza el registro del
    usuario con el ID especificado
    # Retorna un JSON con el usuario actualizado

'''
@app.route("/usuarios/<mail>", methods=["GET"])
def get_usuario(mail):
    ...
    Retorna un JSON con la información del usuario correspondiente al ID proporcionado.
    """
    usuario = Usuario.query.get(mail)  # Obtiene el usuario correspondiente al ID recibido
    return usuario_schema.jsonify(usuario)  # Retorna el JSON del usuario

@app.route("/usuarios/<mail>", methods=["DELETE"])
...
def create_usuario():
    ...
    Lee los datos proporcionados en formato JSON por el cliente y crea un nuevo registro de
    usuario en la base de datos.
    Retorna un JSON con el nuevo usuario creado.
    """
    mail = request.json["mail"]  # Obtiene el mail del usuario del JSON proporcionado
    ...
    new_usuario = Usuario( mail, nombre, password)  # Crea un nuevo objeto Usuario con los datos proporcionados
...
@app.route("/usuarios/<mail>", methods=["PUT"])  # Endpoint para actualizar un usuario
def update_usuario(mail):
    """
    Endpoint para actualizar un usuario existente en la base de datos.

    Lee los datos proporcionados en formato JSON por el cliente y actualiza el registro del
    suario con el ID especificado.
    Retorna un JSON con el usuario actualizado.
    """
    usuario = Usuario.query.get(mail)  # Obtiene el usuario existente con el ID especificado
...
'''
codigo para manejo de productos
'''
...
'''
Este código es el programa principal de la aplicación Flask. Se verifica si el archivo actual
está siendo ejecutado directamente y no importado como módulo. Luego, se inicia el servidor
Flask en el puerto 5000 con el modo de depuración habilitado. Esto permite ejecutar la
aplicación y realizar pruebas mientras se muestra información adicional de depuración en caso de errores.
